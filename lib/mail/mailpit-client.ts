...
/**
 * Mailpit / Inbucket Client for sending local Password Reset emails.
 */
export async function sendResetPasswordEmailMailpit({
  email,
  url,
}: {
  email: string
  ...
  const from = process.env.MAIL_FROM_ADDRESS || "noreply@zap.ch"
  const subject = "Passwort zurücksetzen – ZAP v2"
  const html = `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #e0e0e0; border-radius: 12px;">
      <h2 style="color: #2563eb;">Passwort zurücksetzen</h2>
      <p>Du hast eine Anfrage zum Zurücksetzen deines Passworts bei ZAP v2 gestellt.</p>
      ...
      <p style="color: #666666; font-size: 14px;">Falls du kein neues Passwort angefordert hast, kannst du diese E-Mail einfach ignorieren.</p>
    </div>
  `
  ...
  for (const apiUrl of mailpitApiUrls) {
    try {
      ...
      if (res.ok) {
        const body = await res.json()
        console.log(`[Mailpit API] Reset password email delivered to ${email} via HTTP API (${apiUrl}): ${body?.ID || 'OK'}`)
        return { success: true }
      }
    } catch {
      // Continue to next URL or SMTP fallback
  ...
  try {
    ...
    console.log(`[Mailpit SMTP] Reset password email sent to ${email} via SMTP (${smtpHost}:${smtpPort}): ${info.messageId}`)
    return { success: true }
  } catch (smtpErr) {
    console.warn(`[Mailpit] Local SMTP/HTTP not available (${smtpErr instanceof Error ? smtpErr.message : String(smtpErr)}).`)
    console.log(`\n==================================================`)
    console.log(`[DEV RESET PASSWORD LINK]`)
    console.log(`To: ${email}`)
    console.log(`Link: ${url}`)
    console.log(`==================================================\n`)
