...
// Klassenstufen werden jetzt global über die Navbar gefiltert
...
export default function LernmaterialienPage() {
  ...
  useEffect(() => {
    ...
    async function loadData() {
      ...
      // Lade Subjects - verwende any wegen fehlender Types
  ...
  async function handleDownload(material: Material) {
    ...
    // Update download count via RPC function (bypasses RLS)
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
  ...
  async function handleLinkClick(material: Material) {
    ...
    // Update view count via RPC function (bypasses RLS)
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    await (supabase as any).rpc('increment_material_view_count', { material_id: material.id })
  ...
  const filteredMaterials = materials.filter(material => {
    ...
    // Globaler Klassenfilter
    const matchesClassLevel = !selectedClass || 
      material.class_levels?.includes(selectedClass)
            ...
            <>
              ...
              <div className="absolute right-0 mt-2 w-72 rounded-xl border border-border bg-card shadow-lg z-20 p-4">
                <div className="space-y-4">
                  <div>
                    <label className="text-sm font-medium text-foreground mb-2 block">Fach</label>
                    <select
              ...
              <div className="grid gap-3">
                {subjectMaterials.map(material => {
                  ...
                  return materialIsLink ? (
                    // Notion-style Bookmark für Links
                    <button
                      key={material.id}
