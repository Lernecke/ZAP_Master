...
import { AnmeldungModalDashboard } from './anmeldung-modal-dashboard'
import { IntensivkurseKalender, type KursForKalender } from './kalender'
...
export function IntensivkurseClient({ initialKurse, userProfile }: IntensivkurseClientProps) {
  ...
  // Globaler Klassenfilter aus Context
  ...
  const gefilterteKurse = useMemo(() => {
    ...
    return kurse
      .filter((kurs) => {
        ...
        const matchKlasse = !selectedClass || kurs.klassenstufen.includes(selectedClass)
        
        return istZukuenftig && matchSuche && matchFach && matchKlasse
      })
      .sort((a, b) => {
  ...
  }, [kurse, suchbegriff, fachFilter, sortOrder, selectedClass])
  ...
  return (
    <>
      ...
      {viewMode === 'liste' ? (
        ...
        <IntensivkurseKalender 
          kurse={gefilterteKurse} 
          onKursClick={(kurs: KursForKalender) => {
            setViewMode('liste')
            setExpandedKurs(kurs.id)
          }}
          onAnmelden={(kurs: KursForKalender) => {
