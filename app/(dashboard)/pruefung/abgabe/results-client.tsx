...
export function ResultsClient({ correctAnswers, grade, totalQuestions, answers }: Props) {
  ...
  return (
    <div className="space-y-8">
      <div className={`rounded-2xl p-8 ${passed ? 'bg-green-500/10 border-2 border-green-500/30' : 'bg-red-500/10 border-2 border-red-500/30'}`}>
        ...
        <div className="bg-card rounded-2xl shadow-sm border border-border p-6 md:p-8">
          ...
          <div className="space-y-6">
            {answers.map((answer, index) => (
              <div
                key={index}
                className={`p-4 rounded-xl border-2 ${answer.is_correct ? 'bg-green-500/10 border-green-500/30' : 'bg-red-500/10 border-red-500/30'}`}
              >
                ...
                <div className="grid gap-2 text-sm">
                  ...
                  {answer.is_correct ? (
                    <p className="text-green-600 dark:text-green-400 font-semibold">✓ Richtig</p>
                  ) : (
                    <>
                      <p className="text-red-600 dark:text-red-400 font-semibold">✗ Falsch</p>
                      <p>
                        <span className="text-muted-foreground">Richtige Antwort:</span>{' '}
                        <span className="font-medium text-green-700 dark:text-green-400">{answer.solution}</span>
                      </p>
                    </>
                  )}
                </div>
